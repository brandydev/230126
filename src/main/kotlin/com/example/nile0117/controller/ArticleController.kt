...
class ArticleController {
    ...
    fun getArticlesWithHashtag(
        @RequestParam("text", required = false, defaultValue = "") text: String?
    ): ResponseEntity<*> {
        if (text.isNullOrBlank() || !hashtagRepository.existsByText(text)) {
            return ResponseEntity.ok(
                NileResponse(
                    errorCode = NileCommonError.INVALID_PARAMETER.getErrorCode(),
                    status = NileCommonError.INVALID_PARAMETER.getHttpStatus(),
                    message = "article 검색을 위해서는 hashtag 입력이 필요합니다."
                )
            )
        }

        val articles = articleService.getArticleByHashtag(text)

        return ResponseEntity.ok(
            NileResponse(
                message = "article이 성공적으로 검색되었습니다.",
                result = articles
            )
        )
    }
